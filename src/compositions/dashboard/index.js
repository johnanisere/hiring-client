...
const AdminCards = lazy(() => import('../../components/dashboard/CardsAdmin'));
...
const RemoveHired = lazy(() => import('../../components/hired'));
  ...
  return (
    <ProtectedRoute>
      {role === 'dev' ? (
        ...
        <Layout>
          <Switch>
            <Route exact path={`${match.path}`} component={AdminCards} />
            ...
            <Route
              exact
              path={`${match.path}/usermanagement/dev-settings`}
              component={RemoveHired}
            />
            <Route
              exact
